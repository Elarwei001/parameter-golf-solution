...
app = modal.App("mhc-v2-deep-study")
...
def train_mhc_v2_deep(
    seed: int = 42,
    # 架构参数 - 20 层深模型
    dim: int = 384,  # 稍微减小 dim 以适应更多层
    n_layers: int = 20,
    n_heads: int = 8,
    n_kv_heads: int = 4,
    window_size: int = 192,
    ...
    print("="*70)
    print(f"mHC v2 深层模型研究: {n_layers} 层, dim={dim}")
    print("="*70)
    ...
    print(f"\n模型: {n_params/1e6:.2f}M params, {n_layers} layers, dim={dim}")
    ...
    range_names = ["浅层 (0-4)", "中浅层 (5-9)", "中深层 (10-14)", "深层 (15-19)"]
    
    for (start, end), name in zip(ranges, range_names):
        avg_alpha_attn = np.mean([final_params[i]['alpha_attn'] for i in range(start, end)])
        avg_beta_attn = np.mean([final_params[i]['beta_attn'] for i in range(start, end)])
        avg_alpha_mlp = np.mean([final_params[i]['alpha_mlp'] for i in range(start, end)])
        avg_beta_mlp = np.mean([final_params[i]['beta_mlp'] for i in range(start, end)])
        
        print(f"\n{name}:")
        print(f"  Attention: α={avg_alpha_attn:.3f}, β={avg_beta_attn:.3f}")
        print(f"  MLP:       α={avg_alpha_mlp:.3f}, β={avg_beta_mlp:.3f}")
    
    # 找峰值
    beta_attn_values = [p['beta_attn'] for p in final_params]
    beta_mlp_values = [p['beta_mlp'] for p in final_params]
    
    peak_attn_layer = np.argmax(beta_attn_values)
    peak_mlp_layer = np.argmax(beta_mlp_values)
    
    print(f"\n峰值位置:")
    print(f"  β_attn 峰值: Layer {peak_attn_layer} = {beta_attn_values[peak_attn_layer]:.3f}")
    print(f"  β_mlp 峰值:  Layer {peak_mlp_layer} = {beta_mlp_values[peak_mlp_layer]:.3f}")
    ...
    torch.save({
        ...
        'bpb': bpb,
    }, os.path.join(checkpoint_dir, f"mhc_v2_{n_layers}layers_bpb{bpb:.4f}.pt"))
    ...
    return {
        ...
        'peak_attn_layer': int(peak_attn_layer),
        'peak_mlp_layer': int(peak_mlp_layer),
    }


@app.local_entrypoint()
def main(layers: int = 20, seed: int = 42):
    print("="*70)
    print(f"mHC v2 深层模型研究: {layers} 层")
    print("="*70)
    result = train_mhc_v2_deep.remote(seed=seed, n_layers=layers)
    if result:
        print(f"\n🏁 BPB: {result['bpb']:.4f}")
        print(f"🏁 β_attn 峰值: Layer {result['peak_attn_layer']}")
        print(f"🏁 β_mlp 峰值: Layer {result['peak_mlp_layer']}")
