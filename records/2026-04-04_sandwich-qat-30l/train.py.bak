"""
Sandwich MLP + QAT 30-Layer Experiment
========================================

## Purpose
30-layer version of Sandwich QAT with adaptive QAT switching and FP16 embedding.
Deeper model (30L vs 20L) for better BPB.

## Config
- Layers: 30 (was 20)
- Dim: 384 (unchanged)
- Sandwich MLP: [3.0]*6 + [1.2]*21 + [3.0]*3
- QAT: Ternary (1.58-bit), adaptive switching
- mHC: 4 params/layer, FP32
- FP16 embedding for size efficiency

## Run
```bash
modal run --detach scripts/modal/modal_sandwich_qat_30l.py
```
...
app = modal.App("sandwich-qat-30l")
...
MLP_SCALES_SANDWICH_30L = [3.0]*6 + [1.2]*21 + [3.0]*3
...
def train_sandwich_qat(
    ...
    def save_checkpoint(step, model, optimizer, qat_actual_step, mhc_history, qat_enabled):
        ...
        ckpt_dir = "/data/checkpoints/sandwich_qat_30l"
        os.makedirs(ckpt_dir, exist_ok=True)
        ckpt_path = os.path.join(ckpt_dir, f"sandwich_qat_30l_step{step}.pt")
        torch.save({
            'step': step,
    ...
    # Save results
    checkpoint_dir = "/data/checkpoints/sandwich_qat_30l"
    os.makedirs(checkpoint_dir, exist_ok=True)

    results = {
        'style': 'sandwich_qat_30l_adaptive',
        'config': {
            'dim': dim, 'n_layers': n_layers, 'n_heads': n_heads,
            'n_kv_heads': n_kv_heads, 'local_window': local_window,
    ...
    results_path = os.path.join(checkpoint_dir, f"sandwich_qat_30l_bpb{val_bpb:.4f}.json")
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\n[SAVED] Results: {results_path}")

    ckpt_path = os.path.join(checkpoint_dir, f"sandwich_qat_30l_step{steps}.pt")
    torch.save({
        'step': steps,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
