"""
Modal BPE-8192 Training - Train with larger SentencePiece vocabulary

Usage:
    modal run modal_bpe8k.py::download_data
    modal run modal_bpe8k.py::train_bpe --steps 5000
"""
import modal
import os
import math

app = modal.App("parameter-golf-bpe8k")
...
def download_data():
    """Download SP-8192 tokenizer and data from community repo"""
    ...
    if os.path.exists("/data/datasets/fineweb10B_sp8192") and len(os.listdir("/data/datasets/fineweb10B_sp8192")) > 5:
        print("✅ SP-8192 data already exists!")
        return {"status": "already_exists"}
    
    print("Downloading SP-8192 from sproos/parameter-golf-tokenizers...")
    ...
    # Download tokenizer
    files_to_download = [
        ("tokenizers/fineweb_8192_bpe.model", "/data/tokenizers/fineweb_8192_bpe.model"),
        ("tokenizers/fineweb_8192_bpe.vocab", "/data/tokenizers/fineweb_8192_bpe.vocab"),
    ]
    # Add train shards (10 shards = 1B tokens)
    for i in range(10):
        files_to_download.append((
            f"datasets/fineweb10B_sp8192/fineweb_train_{i:06d}.bin",
            f"/data/datasets/fineweb10B_sp8192/fineweb_train_{i:06d}.bin"
        ))
    # Add val shard
    files_to_download.append((
    ...
    # Verify
    train_files = [f for f in os.listdir("/data/datasets/fineweb10B_sp8192") if "train" in f]
    val_files = [f for f in os.listdir("/data/datasets/fineweb10B_sp8192") if "val" in f]
    print(f"\n✅ Downloaded {len(train_files)} train shards, {len(val_files)} val shards")
    return {"status": "downloaded", "train_shards": len(train_files)}
...
def train_bpe(
    ...
    """Train GPT with BPE-8192 tokenizer on H100"""
    import torch
    ...
    VOCAB_SIZE = 8192  # Larger vocabulary!
    ...
    if not os.path.exists(DATA_DIR):
        ...
        print("Please run: modal run modal_bpe8k.py::download_data")
    ...
    train_data = []
    for f in train_files[:10]:  # Use first 10 shards
    ...
    def calculate_bpb(model, data, num_batches=50):
        ...
        # BPE 8192 on FineWeb: ~4.0 bytes per token (higher compression!)
        BYTES_PER_TOKEN = 4.0
    ...
    print(f"Note: Larger vocab (8192 vs 1024) = more embedding params")
    ...
    LOG_EVERY = 100
    print(f"\n{'='*60}")
    print(f"Training GPT with BPE-8192 on H100")
...
def main():
    print("Parameter Golf - BPE-8192 Training")
    print("=" * 40)
    print("Commands:")
    print("  modal run modal_bpe8k.py::download_data")
    print("  modal run modal_bpe8k.py::train_bpe --steps 5000")
