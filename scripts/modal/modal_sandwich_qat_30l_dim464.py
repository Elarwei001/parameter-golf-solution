"""
Sandwich MLP + QAT 30L dim=464 Experiment
===========================================

30L Sandwich QAT with dim=464 (max ~16MB with FP16 embedding) and 40k steps.

## Run
```bash
modal run --detach scripts/modal/modal_sandwich_qat_30l_dim464.py
```
...
app = modal.App("sandwich-qat-30l-dim464")
...
def train_sandwich_qat(
    ...
    def save_checkpoint(step, model, optimizer, qat_actual_step, mhc_history, qat_enabled):
        ...
        ckpt_dir = "/data/checkpoints/sandwich_qat_30l_dim460"
        os.makedirs(ckpt_dir, exist_ok=True)
        ckpt_path = os.path.join(ckpt_dir, f"sandwich_qat_30l_dim460_step{step}.pt")
        torch.save({
            'step': step,
    ...
    # Save results
    checkpoint_dir = "/data/checkpoints/sandwich_qat_30l_dim460"
    os.makedirs(checkpoint_dir, exist_ok=True)

    results = {
        'style': 'sandwich_qat_30l_dim460_adaptive',
        'config': {
            'dim': dim, 'n_layers': n_layers, 'n_heads': n_heads,
            'n_kv_heads': n_kv_heads, 'local_window': local_window,
    ...
    results_path = os.path.join(checkpoint_dir, f"sandwich_qat_30l_dim460_bpb{val_bpb:.4f}.json")
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\n[SAVED] Results: {results_path}")

    ckpt_path = os.path.join(checkpoint_dir, f"sandwich_qat_30l_dim460_step{steps}.pt")
    torch.save({
        'step': steps,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
