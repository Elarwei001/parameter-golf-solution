"""
渐进式 Attention 实验
基于 mHC 发现：浅层 attention 权重很低 (α ≈ 0.2-0.5)
尝试：浅层去掉 attention，把参数省给深层
"""
import modal
import os
import math

app = modal.App("progressive-attention")
...
def train_progressive_attention(
    seed: int = 42,
    # 架构参数
    dim: int = 416,
    n_layers: int = 11,
    ...
    window_size: int = 192,
    # 渐进式参数
    mlp_only_layers: int = 3,  # 前 N 层只用 MLP
    # 训练参数
    lr: float = 1e-3,
    batch_size: int = 64,
    seq_len: int = 256,
    ...
    print("="*70)
    print(f"渐进式 Attention 实验")
    print(f"前 {mlp_only_layers} 层: 只有 MLP (无 Attention)")
    print(f"后 {n_layers - mlp_only_layers} 层: 正常 Attention + MLP")
    print("="*70)
    ...
    class MLPOnlyBlock(nn.Module):
        """只有 MLP 的 block（无 Attention）"""
        def __init__(self, dim, hidden_mult=3):
            super().__init__()
            # 用更大的 MLP 来补偿没有 attention
            self.mlp = MLP(dim, dim * hidden_mult)
            self.ln = RMSNorm(dim)
    ...
    class ProgressiveGPT(nn.Module):
        def __init__(self, vocab_size, dim, n_layers, n_heads, n_kv_heads, 
            ...
            for i in range(n_layers):
                if i < mlp_only_layers:
                    # 浅层：只有 MLP
                    self.layers.append(MLPOnlyBlock(dim, hidden_mult=3))
                else:
                    # 深层：完整 Attention + MLP
                    self.layers.append(FullBlock(dim, n_heads, n_kv_heads, window_size))
    ...
    # 计算参数分布
    mlp_only_params = sum(p.numel() for layer in model.layers[:mlp_only_layers] 
                          for p in layer.parameters())
    full_block_params = sum(p.numel() for layer in model.layers[mlp_only_layers:] 
                            for p in layer.parameters())
    
    print(f"\n模型架构:")
    print(f"  总参数: {n_params/1e6:.2f}M")
    print(f"  前 {mlp_only_layers} 层 (MLP only): {mlp_only_params/1e6:.2f}M")
    print(f"  后 {n_layers - mlp_only_layers} 层 (Full): {full_block_params/1e6:.2f}M")
    ...
    print(f"\n{'='*70}")
    print(f"🏆 结果 (渐进式 Attention)")
    print(f"{'='*70}")
    print(f"  架构: {mlp_only_layers} MLP-only + {n_layers - mlp_only_layers} Full")
    ...
    checkpoint_path = os.path.join(checkpoint_dir, f"progressive_{mlp_only_layers}mlp_bpb{bpb:.4f}.pt")
    torch.save({
...
def main(mlp_layers: int = 3, seed: int = 42):
    print("="*70)
    print(f"渐进式 Attention 实验 - 前 {mlp_layers} 层只用 MLP")
    print("="*70)
    result = train_progressive_attention.remote(seed=seed, mlp_only_layers=mlp_layers)
    if result:
        print(f"\n🏁 BPB: {result['bpb']:.4f}")
        print(f"🏁 参数: {result['n_params']/1e6:.2f}M")
