"""
Modal BPE Training - Train with SentencePiece BPE tokenizer

Usage:
    # First download data (if not already done)
    modal run modal_bpe.py::download_data
    
    # Run training with BPE
    modal run modal_bpe.py::train_bpe --steps 5000
"""
import modal
import os
import math

app = modal.App("parameter-golf-bpe")
...
def download_data(variant: str = "sp1024", train_shards: int = 20):
    """Download FineWeb dataset with BPE tokenizer"""
    import subprocess
    import shutil
    
    # Check if already downloaded
    if os.path.exists("/data/datasets/fineweb10B_sp1024") and os.path.exists("/data/tokenizers"):
        print("✅ Data already exists!")
        print(f"Datasets: {os.listdir('/data/datasets')}")
        print(f"Tokenizers: {os.listdir('/data/tokenizers')}")
...
def train_bpe(
    ...
    """Train GPT with BPE-1024 tokenizer on H100"""
    import torch
    ...
    def calculate_bpb(model, data, num_batches=50):
        ...
        # BPE 1024 on FineWeb: ~2.3 bytes per token (compression ratio)
        BYTES_PER_TOKEN = 2.3
    ...
    LOG_EVERY = 100
    print(f"\n{'='*60}")
    print(f"Training GPT with BPE-1024 on H100")
...
def main():
    print("Parameter Golf - BPE Training")
    print("=" * 40)
    print("Commands:")
    print("  modal run modal_bpe.py::download_data")
    print("  modal run modal_bpe.py::train_bpe --steps 5000")
