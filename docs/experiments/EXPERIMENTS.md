...
| Day 1 | LeakyReLU2 | 2.18 | -4.3% | 激活函数很重要 |
...
| Day 4 | Sandwich QAT Adaptive | 1.5321 | +1.97% vs FP32 | 自适应 QAT 切换，仍比 FP32 差 |
| Day 4 | Embedding FP16 转换 | 1.5336 | +0.16% vs FP32 emb | **省 6.29MB，几乎无损!** |
...
## 2026-04-03: mHC 20层/32层深度实验

### 实验目的
验证 mHC α/β 分布规律在更深模型中是否一致
...
**结果**:
- Val BPB: **1.4978**
- β_attn 峰值: Layer 19 (β=1.049)
- β_mlp 峰值: Layer 4 (β=0.864)
...
**这不是最后一层**,而是稳定出现在 ~85-95% 深度!

**可能解释**:
1. 预输出整合层 - 模型需要专门层来整合特征
2. 信息瓶颈点 - 压缩特征到预测 logits
3. Weight tying 反向影响 - 输出层梯度影响

### 待验证
- [ ] seed=43 跑 32 层,确认 Layer 27 异常是否稳定复现
- [ ] 测试 48L 或 64L,看异常是否出现在 ~Layer 41-43 或 ~Layer 55-58

**可视化**: `results/figures/mhc_deepseek_32L_analysis.png`
...
### 实现
添加到 `scripts/modal/modal_mhc_v2.py`:

| 指标 | 采集频率 | 用途 |
|------|----------|------|
| train_loss | 每步 | Loss 曲线 |
| val_loss | 每 500 步 | 判断过拟合 |
| α/β 值 (所有层) | 每 100 步 | 参数收敛轨迹 |
| 每层梯度范数 | 每 500 步 | 看哪些层在学 |

### 输出格式
- JSON 文件: `metrics_{n_layers}L_seed{seed}.json`
- 包含: config, train_loss, val_loss, mhc_params, grad_norms, final_results

---

## 2026-04-03: Repo 结构整理
...
2. ✅ ~~数据过滤~~ - 负面结果
3. ✅ ~~EMA~~ - 负面结果
4. ✅ ~~渐进式 Attention~~ - 参数减 6.5%,BPB 差 0.25%
5. ✅ ~~mHC 20L/32L~~ - 发现 ~90% 深度异常规律
6. 🔄 **32L seed=43** - 验证异常复现性 (进行中)
7. **层级差异化** - 浅层少参数,深层多参数
8. **48L/64L** - 验证更深模型的异常层位置
...
| Val BPB | **1.5315** | 1.5025 | **+1.93%** |
| 参数量 | 30.25M | 32.85M | -7.9% |

**结论**：PLE 比基线差 1.93%，不是之前报告的 +274%。PLE 本身有轻微负面影响，不采用。

**可能原因**：
1. PLE 干扰了残差流 — 每层注入嵌入信号打乱特征传播
2. 和权重共享冲突 — 输出层复用主嵌入权重，额外 PLE 信号造成混淆
3. 简单加法注入不够 — 可能需要门控机制
...
## 2026-04-03: Alternating Attention 实验

**技术**：
- 偶数层 (0,2,4,...) + 最后一层: Global full attention
- 奇数层 (1,3,5,...): Local sliding-window attention (window=128)
- 灵感来源: Gemma 3n / gamma4 建议

**假设**：Local attention 计算量更小，通过 Local+Global 交替，可以在相同计算成本下使用更大的模型。
...
| Layers | 20 |
| Dim | 384 |
| Global layers | 11 (偶数层 + 最后一层) |
| Local layers | 9 (奇数层, window=128) |
| Params | 29.70M |
| Steps | 5000 |
...
### Front-loaded MLP: 深层也降到 1.2× (2026-04-04)

**配置**：层 0-3 MLP 3×, 层 4-19 MLP 1.2×（深层不恢复）

**结果**：

| 配置 | Params | Val BPB | vs Uniform |
|------|--------|---------|-------------|
| Uniform (all 3x) | 29.70M | **1.4777** | - |
| Sandwich (3x/1.2x/3x) | 22.79M | 1.4833 | +0.38% |
| **Front (3x/1.2x/1.2x)** | **21.20M** | 1.4918 | **+0.95%** |

**自洽性悖论的解答**：假设“beta_mlp 下降意味着深层 MLP 不重要”是**错误的**。

虽然 beta_mlp 确实在下降，但深层 3× MLP 仍然有意义贡献。原因：beta_mlp 衡量的是**相对重要性**，而非**绝对必要性**。一个大的 beta × 小 MLP 可能不如一个小的 beta × 大 MLP。

**mHC 参数洞察**：
- beta_mlp 在 Sandwich 和 Front 之间几乎一致——MLP 尺度不影响模型对 MLP 的依赖程度
- Front 在层 4-5 出现 beta_attn 飙升（0.955 vs Sandwich 的 0.733），这是**补偿机制**——中段 attention 加大弥补深层 MLP 不足
- Front 在层 18 beta_attn 降到 0.454（vs Sandwich 的 0.660），深层信号衰减

**结论**：Sandwich (3x/1.2x/3x) 是最优 MLP 配置。Front 过度压缩了深层。

可视化：![mHC 对比图](reports/alternating-attention/mhc_sandwich_vs_front.png)

详见：[Front-loaded 报告](reports/alternating-attention/2026-04-04_alt-a-mhc-front.md)

### Sandwich + QAT 实验 (2026-04-04)

**配置**: Sandwich MLP (3x/1.2x/3x) + Ternary QAT (1.58-bit) from step 0 + mHC from scratch

**结果**: BPB **1.5350** (+3.5% vs Sandwich FP32, 甚至比 baseline 还差)

| 配置 | BPB | Quantized Size |
|------|-----|---------------|
| Sandwich FP32 | 1.4833 | N/A |
| **Sandwich + QAT** | **1.5350** | **16.52 MB** |

**关键发现**:
1. **Embedding 是瓶颈**: FP32 embedding 占 12.60MB (16MB 的 78.8%)，留给量化权重的空间只有 3.40MB
2. **从 step 0 开始 QAT 太激进**: 模型没有机会先学好表示，就被量化噪声干扰
3. **mHC 演化**: QAT 下 beta_mlp 系统性高于 FP32，模型更依赖 MLP 来补偿量化损失
4. **beta_mlp 在 QAT 下更稳定**: 各步之间变化很小，说明 QAT 限制了 mHC 的探索空间

可视化：![mHC QAT 演化图](reports/alternating-attention/mhc_qat_evolution.png)

**日志截断问题**: `modal app logs` 默认只显示最近约 80 行。需要用 `--tail 500` 获取完整日志。

详见：[Sandwich QAT 报告](reports/alternating-attention/2026-04-04_sandwich-qat.md)

### Sandwich QAT Adaptive (2026-04-04)

**配置**: Sandwich MLP + Ternary QAT (1.58-bit) + 自适应切换 + mHC from scratch

**核心改进**: 先 FP32 训练，等 loss 收敛后自动切换 QAT（自适应切换算法）

| 配置 | BPB | Quantized Size |
|------|-----|---------------|
| Sandwich FP32 (baseline) | 1.5025 | N/A |
| Sandwich + QAT (from step 0) | 1.5350 | 16.52 MB |
| **Sandwich + QAT Adaptive** | **1.5321** | **16.52 MB** |

**自适应 QAT 算法**: EMA loss 收敛率 < 0.001 时切换，实际在 step 500 触发（warmup 下限）

**结论**: 自适应 QAT 比 from-step-0 好 0.19% BPB，但仍比 FP32 baseline 差 +1.97%

详见：[实验报告](../../experiment_sandwich_qat_adaptive.md)

### FP16 Embedding 转换 (2026-04-04)

**配置**: Sandwich QAT 模型，仅将 embedding 从 FP32 转为 FP16

| 格式 | Val Loss | Val BPB | Quantized Size |
|------|----------|---------|---------------|
...
**关键发现**:
1. Embedding FP16 只损失 +0.16% BPB，几乎无损
2. 省下 6.29 MB，模型从 16.52MB 降到 10.23MB
3. 这为升级到更大 dim 留出空间：dim=478, 45M params → 15.98 MB ✅

详见：[实验报告](../../experiment_sandwich_qat_adaptive.md) 的 "Embedding-only FP16 Conversion" 章节
...
|------|-----|---------------|
| Sandwich FP32 | 1.4833 | N/A |
| QAT from step 0 | 1.5350 | 16.52 MB |
| **QAT Adaptive** | **1.5321** | **16.52 MB** |

**关键发现**:
1. 自适应切换 (step 500) 比从 step 0 开 QAT 好 0.19% BPB
2. 但仍比 FP32 baseline 差 +1.97%
3. 模型 16.52MB 超过 16MB 限制

详见：[实验报告](../../experiment_sandwich_qat_adaptive.md)

### FP16 后处理转换 (2026-04-04)

| 格式 | Val Loss | Val BPB | 模型大小 |
|------|----------|---------|----------|
| FP32 (QAT) | 3.9009 | 1.5335 | 91.18 MB |
| 全模型 FP16 | 3.9087 | 1.5365 | 45.59 MB |
| Delta | +0.0078 | +0.20% | -50% |

**结论**: FP16 转换整个模型损失仅 +0.20% BPB

### Embedding FP16 转换 (2026-04-04)

**配置**: 仅将 embedding 从 FP32 转为 FP16，其余保持不变

| 格式 | Val Loss | Val BPB | 量化大小 |
|------|----------|---------|----------|
| FP32 embedding | 3.8949 | 1.5311 | 16.52 MB |
| **FP16 embedding** | **3.9011** | **1.5336** | **10.23 MB** |
| Delta | +0.0062 | **+0.16%** | **-6.29 MB** |

**关键发现**:
1. Embedding FP16 几乎无损，仅 +0.16% BPB
2. 省下 6.29 MB，模型从 16.52MB 降到 10.23MB
3. 这为升级到更大 dim 留出空间：dim=478, 45M params → 15.98 MB ✅
...
**配置**：
- FlashAttention 2 + sliding window
- seq_len = 1024 (比当前 256 长 4 倍)
- Alternating: Global 层用 `flash_attn_func`，Local 层用 `window_size=(128, 0)`

**目的**：
- 验证真正的稀疏计算能否让长上下文训练可行
- 看长上下文对 BPB 是否有帮助
